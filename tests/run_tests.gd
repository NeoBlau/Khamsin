...
func _scrub(kept: Array[Node]) -> void:
	...
	# Следы в песке — глобальное состояние, и живут они, пока их не стёрли.
	# Стенды почти все ставят машину в начало координат, так что без уборки
	# каждый следующий тест начинается в колее предыдущего: разгон выходит
	# вдвое медленнее, спущенные колёса «перестают помогать», и симптом ничем
	# не выдаёт причину. Мир между тестами не пересобирается, а песок обязан.
	World.sand.clear()
